...
PYBIND11_PLUGIN(passingstring)
{
	...
	py::module m("passingstring", "pybind11 example plugin - passing string to Fortran");

	m.def("cpp_dump", &cpp_dump, "A function which prints a string from C++.");
	m.def("fortran_dump", [](const std::string& s) {fortran_dump(s); }, "A function which prints a string from Fortran.");
