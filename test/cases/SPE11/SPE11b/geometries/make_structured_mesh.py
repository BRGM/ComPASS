...
class StructuredLattice:
    ...
    def corners(self, cell: tuple) -> tuple:
        def _get_quad_corners(p0: int) -> tuple:
            return (p0, p0 + 1, p0 + 1 + num_cells[0] + 1, p0 + num_cells[0] + 1)
    ...
    def center(self, cell: tuple) -> tuple:
        ...
        for pidx in self.corners(cell):
            result = tuple(
                [result[i] + self._points[pidx][i] for i in range(self._dim)]
            )
        return tuple(
            [(result[i] / len(corners) if i < self._dim else 0.0) for i in range(3)]
        )
...
class PhysicalIndexMapper:
    ...
    def physical_groups(self, dim: int) -> dict:
        return {
            self._get_physical_name(dim, tag): tag for _, tag in self._physical_groups
        }
    ...
    def _project_to_model_for_index_queries(self, position: tuple) -> tuple:
        if self._variant == "C":
            return (position[0], position[2] - z_offset_at(position[1]), 0.0)
        return position
    ...
    def _get_entity_physical_index(self, dim: int, tag: int) -> typing.Optional[int]:
        for (
            physical_dim,
            physical_index,
        ), entity_tags in self._physical_groups.items():
...
class FilteredLattice:
    ...
    def corners(self, cell: tuple) -> typing.Iterable:
        return (
            self._point_map_from_lattice[p_idx] for p_idx in self._lattice.corners(cell)
        )
    ...
    def _create_mappings(self) -> None:
        self._cell_index_map = [
            i for i in range(self._lattice.number_of_cells) if self._is_included(i)
        ]
        self._cells = [
            cell for i, cell in enumerate(self._lattice.cells) if self._is_included(i)
        ]
        ...
        self._point_map_from_lattice = list(
            itertools.accumulate(included_points, initial=0)
        )[:-1]
        self._point_map_to_lattice = [
            i for i in range(self._lattice.number_of_points) if included_points[i]
        ]
        self._points = [
...
parser = argparse.ArgumentParser(
    description="Create a structured gmsh grid for one of the SPE11 variants"
)
parser.add_argument(
    "-v",
    "--variant",
...
parser.add_argument(
    ...
    "--number-of-cells-x",
    required=True,
    help="Desired number of cells in x-direction",
)
parser.add_argument(
    "-ny",
    "--number-of-cells-y",
    required=True,
    help="Desired number of cells in y-direction",
)
parser.add_argument(
    "-nz",
    "--number-of-cells-z",
    required=False,
    help="Desired number of cells in z-direction",
)
...
if variant == "C":
    if args["number_of_cells_z"] is None:
        sys.exit(
            "Variant C is three-dimensional, please provide the number of cells in z-direction"
        )
    if not os.path.exists(_get_variant_geo_file(variant)):
        print(
            "Could not find geometry of version C. Invoking the generator script to create it."
        )
        subprocess.run(["python3", "make_spe11c_geo.py", "-s", "100"], check=True)
        assert os.path.exists(_get_variant_geo_file(variant))

num_cells = tuple(
    [int(args[f"number_of_cells_{['x', 'y', 'z'][i]}"]) for i in range(dim)]
)
gmsh_cell_type = 3 if dim == 2 else 5  # quadrangle,  # hexahedron

physical_index_mapper = PhysicalIndexMapper(variant)
lattice: StructuredLattice | FilteredLattice = StructuredLattice(
    *_get_bounding_box(gmsh.model), num_cells=num_cells
)
...
for cell_index, cell in enumerate(lattice.cells):
    ...
    physical_cell_indices[cell_index] = physical_index_mapper.physical_index(
        lattice.center(cell)
    )

print("Removing all cells outside of the domain")
inside_lattice = FilteredLattice(
    lattice, physical_cell_indices, PHYSICAL_INDEX_OUTSIDE_OF_DOMAIN
)
...
if args["remove_cells_in_facies_7"]:
    ...
    filtered_lattice = FilteredLattice(
        inside_lattice, inside_physical_cell_indices, PHYSICAL_INDEX_FACIES_7
    )
...
with open(msh_file_name, "w") as msh_file:
    msh_file.write(
        ...
        {}
    """.format(
                len(physical_index_mapper.physical_groups(dim))
            ).lstrip(
                "\n"
            )
        )
