...
well_depth: float = 40  # m (change if you change the mesh !!!)
...
mesh = vtk2RawMesh("./geometries/spe11b_2d_refined_20_extruded.vtu")
# change the depth
pts = mesh.vertices
# change the depth (y direction)
pts[pts[:, 1] == 1, 1] = well_depth
# translate the z axis
pts[:, -1] += 1200
rocktype = mesh.attribute

# identify the nodes in Dirichlet rocktype cells
dir_rkt = (rocktype >= 2) & (rocktype <= 5)
dir_nodes_mask = np.zeros(len(pts), dtype=bool)
dir_nodes_mask[np.unique(mesh.cell_nodes[dir_rkt])] = True
...
simulation.postprocess()
# export also the densities...
export_states(ComPASS.to_output_directory("states_1100yr"))
