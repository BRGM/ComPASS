...
def dump(k):
    ...
    MT.to_vtu(
        mesh, "surface%03d.vtu" % k, celldata={"component": Sk.connected_components()}
    )
    print("dumped", k)
...
for k, S in enumerate(surfaces):
    ...
    mesh = MT.TSurf.make(*S.as_arrays())
    MT.to_vtu(
        mesh, "surface%03d.vtu" % k, celldata={"component": S.connected_components()}
    )
