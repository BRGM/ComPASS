...
""" Dirichlet nodes at the bottom and top """
r = 2.625
y_min = -20
y_max = 20
z_min = -10
...
bottom_node = [abs(z - z_min) <= eps for _, _, z in nodes]
bottom_flag = 3
top_node = [abs(z - z_max) <= eps for _, _, z in nodes]
top_flag = 2
...
""" Initial, Freeflow and Dirichlet values """
# select freeflow faces
fc = simulation.compute_face_centers()
simulation.set_freeflow_faces([y * y + z * z <= r * r + eps for _, y, z in fc])
...
TGal = 303.0
HurGal = 0.5
simulation.set_atm_temperature(TGal)
simulation.set_atm_rain_flux(0.0)
simulation.set_atm_pressure(PgGal)
