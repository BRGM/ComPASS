...
if not ComPASS.mpi.is_on_master_proc:
    grid = Dirichlet_node = fracture_faces = set_global_flags = select_global_rocktype = None
...
def set_variable_boundary_heat_flux():
    ...
    Neumann.molar_flux[:] = liq_molar_fraction_qbin[:]*qbin   # two components (air, water)
    Neumann.heat_flux = qbin * ComPASS.liquid_molar_enthalpy(p0,T0+20.,liq_molar_fraction_qbin)
