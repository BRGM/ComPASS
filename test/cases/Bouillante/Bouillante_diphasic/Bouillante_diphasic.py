...
fracture_thermal_cond = 2.        # fracture thermal conductivity
...
x_source, y_source, radius = 634.E3, 1781.9E3, 1.E3  # coordinate and radius of the source center, almost in one fracture
x_top_diphasic, y_top_diphasic, top_diphasic_radius =  640.E3, 1794.E3, 4.E3 # coordinate and radius of the mountain (xmax, ymax, radius) top diphasic
...
if ComPASS.mpi.is_on_master_proc:
    ...
    mesh = MT.TetMesh.make(c3t3.vertices, MT.idarray(c3t3.cells))    # meshtool objects readable by Compass
    ...
    def fracture_faces():
        sys.stdout.write('selecting fractures faces for:' + str(faults_facets.keys()) + '\n')
    ...
    def set_global_flags():
        ...
        for nodes in toponodes:  # inutile: fait 3 fois le test sur chaque noeud car boucle sur les top mailles...
            for nn in nodes:
                xyz_topo = c3t3.vertices[nn]
                if xyz_topo[2] < 0. :
                    nodeflags[nn] = sea_flag
                elif (xyz_topo[0] - x_top_diphasic)**2 + (xyz_topo[1] - y_top_diphasic)**2 < top_diphasic_radius**2:
...
if not ComPASS.mpi.is_on_master_proc:
    mesh = Dirichlet_node = fracture_faces = set_global_flags = select_global_rocktype = None
...
def set_Dirichlet_state(state, pts): #top nodes
    ...
    state.p[node_flags==sea_flag] = lininterp(abs(pts[node_flags==sea_flag,2]), p0, 800*gravity)  # z is negatif as in sea
    state.T[node_flags==sea_flag] = np.maximum(degC2K(5),lininterp(pts[node_flags==sea_flag,2], T0, 0.22))
    state.S[node_flags==sea_flag] = [0, 1]
    state.C[node_flags==sea_flag] = [[ 1., 0.], [0., 1.]]
    # monophasic gas
    ...
    state.C[node_flags==top_gas_flag] = [[ .99, .01], [0., 1.]] # air, water
    # diphasic
    ...
    state.C[node_flags==top_diphasic_flag] = [[ 1., 0.], [0., 1.]] #[[ .97, .03], [1.E-3,.999]]
...
def set_variable_initial_bc_values():
    ...
    set_fracture_state(ComPASS.fracture_states(), dtm.depths(ComPASS.compute_fracture_centers()))
