...
x_top_diphasic, y_top_diphasic, top_diphasic_radius =  640.E3, 1794.E3, 4.E3 # coordinate and radius of the mountain (xmax, ymax, radius) top diphasic : not possible because only water
...
def set_Dirichlet_state(state, pts): #top nodes
    ...
    state.context[:] = 2
    # sea
    state.p[node_flags==sea_flag] = lininterp(pts[node_flags==sea_flag,2], p0, -800*gravity)
    Ttemp = []
    ...
    state.C[node_flags==top_gas_flag] = 1.
    # diphasic
    state.p[node_flags==top_diphasic_flag] = p0
    state.T[node_flags==top_diphasic_flag] = T0
    state.S[node_flags==top_diphasic_flag] = [0, 1] #[.72, .28] il n'y a pas encore de gaz
    state.C[node_flags==top_diphasic_flag] = 1.
