...
inflow1 = lambda x, y: (y >= grid.origin[1]+grid.extent[1]) & ( grid.origin[0] + 0.3 <= x) & (x <= grid.origin[0]+0.4)
inflow2 = lambda x, y: (x <= grid.origin[0]) & ( grid.origin[1]+0.3 <= y) & (y <= grid.origin[1]+0.4 )
outflow = lambda x, y: (x >= grid.origin[0]+grid.extent[0] ) & (grid.origin[1]+0.2 <= y) & (y <= grid.origin[1]+0.4)
...
def cell_permeability_factory(grid):
    def cell_permeability():
        ...
        obstacle = (grid.origin[0] + 1 <= xc) & (xc <= grid.origin[0] + 1.1) & (yc <= grid.origin[1] + 0.9)
