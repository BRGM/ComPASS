...
class Coupling (object) :
	def __init__(self, Chobj, TrObj, nb_time_steps):
		...
		self.jacCh = np.zeros(((TrObj.nb_points)*Chobj._Nc, (TrObj.nb_points)*Chobj._Nc))
	...
	def meth_F(self, Y, Cold, Fold, csol, Chobj, Trobj, t, ts_manager) :
		...
		return ff.flatten(), Cnew.flatten(), self.jacCh ### pas besoin de retourner jacCh si on le garde comme attribut de classe
	...
	def Simul_time_loop(self, xr, t, final_time,  ts_manager, meth, Chobj, Trobj) :
		...
		if (meth.nom == "meth_CFT"):
		    Y0 = np.append(self.C.flatten(), np.append(self.Tot.flatten(), self.F.flatten()))
		...
		while t<final_time : # boucle sur les pas de temps
		    ...
		    if (meth.nom == "meth_CFT"):
		    	Y, info = nonlin_solve(lambda Y:self.meth_CFT(Y,Cold,Fold, csol, Chobj, Trobj, t, ts_manager), Y, jacobian='couplingJacCFT', verbose=1) 
		    elif (meth.nom == "meth_CF"):
		    	Y, info = nonlin_solve(lambda Y:self.meth_CF(Y,Cold,Fold, csol, Chobj, Trobj, t, ts_manager), Y, jacobian='couplingJacCF', verbose=1) 
		    elif (meth.nom == "meth_F"):
		    	Y, info = nonlin_solve(lambda Y:self.meth_F(Y,Cold,Fold, csol, Chobj, Trobj, t, ts_manager), Y, jacobian='couplingJacF', verbose=1) 
