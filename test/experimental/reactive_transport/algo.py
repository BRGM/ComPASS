...
def _is_public_name(name):
    ...
    return _PUBLIC_NAME_RE.match(name) \
...
class Struct(object):
    ...
    def __setattr__(self, name, val):
        ...
        if _is_public_name(name):
            if dtype is None:
                raise AttributeError("Can not add attribute {} to Struct"
                                     .format(name))
            elif not isinstance(val, dtype):
                if dtype == float  and  isinstance(val, int):
                    val = float(val)
                else:
                    raise TypeError("Attribute {} of Struct {} must be {}"
    ...
    def __init__(self, **kw):
        ...
        if missing:
            ...
            raise ValueError("Missing attribute(s) {} in Struct {}"
