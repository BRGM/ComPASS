...
class Transport(object):
    ...
    def plot_1D_concentrations(self, t, conc):
        ...
        for i in range(len(Caq)):
            ...
            cs = plt.plot(
                x[ligne : ligne + nx], Ci_nodes[ligne : ligne + nx], label=Caq[i]
            )
            ...
            cs = plt.plot(
                x[ligne : ligne + nx], Ci_cells[ligne : ligne + nx], label=Caq[i]
            )
        ...
        plt.pause(0.1)
        plt.savefig(
            ComPASS.to_output_directory("1D_cell_conc_at_time_" + str(t)), format="png"
        )
        # if ((t==0)|(t==21600)|(t==28800)) :
