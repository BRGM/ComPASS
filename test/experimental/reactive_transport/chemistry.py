...
class Chemistry(object):
    ...
    def fixed_free(self):
        ...
        if len(fixed) > 0:
            logKff = np.transpose([self.logK]) + self.StMat[:, fixed] * np.log(
                self.rhs[fixed]
            )
        else:
            logKff = np.transpose([self.logK])
    ...
    def fJacChimie(self, z, rhs, StMat, logKff):
        ...
        jac = -np.diag(fcs[0]) - np.matmul(
            np.matmul(np.transpose(StMat), np.diag(tmp.reshape(1, tmp.size)[0])), StMat
        )
    ...
    def solve(self):
        ...
        zsol = optimize.root(
            self.fJacChimie,
            self.zin,
            args=(self.rhs[free], self.StMat[:, free], logKff),
            jac=True,
            method="lm",
            options={"ftol": 1.49012e-08},
        )
        ...
        niter = (
            zsol.njev
        )  # zsol.nit number of iterations #TO DO : to get from the program as result specially for 'lm' and 'hybr' methods
    ...
    def post_process(self, zzsol):
        ...
        [fixed, free, logKff] = self.fixed_free()
        [ff, jac] = self.fJacChimie(
            zzsol[free], self.rhs[free], self.StMat[:, free], logKff
        )
        ...
        psi_prime_dT = np.matmul(
            np.matmul(
                np.matmul(np.transpose(self.A), np.diagflat(y)),
                np.block([[self.A, self.B]]),
            ),
            X,
        )
