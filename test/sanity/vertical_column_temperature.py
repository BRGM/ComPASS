...
standard_loop(initial_timestep= 1 * day, final_time = final_time, nitermax=140)

T = ComPASS.cell_states().T
zc = ComPASS.cell_centers()[:, 2]
#print(np.linalg.norm(T0-zc*bottom_heat_flux/K_matrix-T, np.inf)<1e-2)
#print(ComPASS.get_current_time() < final_time)
if ( ComPASS.get_current_time() < final_time or
