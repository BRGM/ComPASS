...
def calculate_fugacity_coeff(T, P):
    ...
    return eos.fugacity_l / bar
    # if eos.phase == "g":
    # return eos.fugacity_g/bar, eos.dfugacity_g_dP/bar, eos.dfugacity_g_dT/bar
    # elif eos.phase == "l":
    # return eos.fugacity_l/bar#, eos.dfugacity_l_dP/bar, eos.dfugacity_l_dT/bar


def calculate_density(T, P):
    eos = SRK(Tc=304.13, Pc=73.77 * bar, omega=0.22394, T=T, P=P * bar)
    if eos.phase == "g":
        return eos.rho_g, eos.drho_dP_g, eos.edrho_dT_g
    elif eos.phase == "l":
...
def calculate_enthalpy(T, P):
    eos = SRK(Tc=304.13, Pc=73.77 * bar, omega=0.22394, T=T, P=P * bar)
    if eos.phase == "g":
        return eos.H_dep_g, eos.dH_dep_dP_g, eos.dH_dep_dT_g
    elif eos.phase == "l":
...
def plot_results():
    ...
    T = 320  # Example of temperature

    for ax, property_name in zip(axes.flat, ["Fugacity", "Density", "Enthalpy"]):
        ax.set_ylabel(property_name)
        ax.set_xlabel("Pressure (Bar)")

    # Fugacity comparison
    fg_compass = np.vectorize(
        lambda T, x: s.cpp_liquid_CO2_fugacity(x * bar, T, np.array([1, 0]))
    )
    ...
    # Density comparison with CO2 noted gas but with liquid properties
    ...
    energy_compass = np.vectorize(
        lambda T, x: s.cpp_liquid_molar_enthalpy(x * bar, T, np.array([1, 0])) * 1.0e-3
    )
