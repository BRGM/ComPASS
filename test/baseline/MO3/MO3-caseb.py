...
# Set initial values
Xliq = simulation.build_state(simulation.Context.diphasic, p=30.5 * bar, Sg=0)
Xgas = simulation.build_state(simulation.Context.diphasic, p=30.5 * bar, Sg=1)
xyz = simulation.vertices()
z = xyz[:, 2]
simulation.node_states().set(z <= 0.1, Xliq)
simulation.node_states().set(z > 0.1, Xgas)
xyz = simulation.compute_cell_centers()
z = xyz[:, 2]
simulation.cell_states().set(z <= 0.1, Xliq)
simulation.cell_states().set(z > 0.1, Xgas)
