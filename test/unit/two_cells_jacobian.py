...
A = simulation.retrieve_jacobian()

ij = A.block_indexes()
for ij, block in zip(A.block_indexes(), A.blocks()):
    print("Block at:", tuple(ij), ":", [tuple(row) for row in block])

S = simulation.retrieve_second_member()
print(S)
