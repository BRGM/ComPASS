...
auto part_graph(COC& adjacencies, idx_t nb_parts, const bool Kway)
{
    ...
    static_assert(std::is_same<idx_t, COC::value_type>::value, "unconsistent types");
    ...
    auto status = method(
        &nb_vertices, &nb_constraints,
         adjacencies.mutable_offset_data(), adjacencies.mutable_content_data(),
        nullptr, nullptr, nullptr,
        &nb_parts,
        nullptr, nullptr,
        options, &edge_cut, result.mutable_data()
    );
...
void add_Metis_wrapper(py::module& module)
{
    ...
    module.def(
        "metis_part_graph", &part_graph,
        "call Metis part graph methods, defaults to recursive algorithm",
        py::arg("adjacencies"), py::arg("nb_parts"), py::arg("Kway") = false
    );
