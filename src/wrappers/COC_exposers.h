...
struct COC_container_factory<Value_type, true> {
    static auto make(py::module& module, std::string basename) {
        return py::class_<GenericCOC_container<Value_type>>(module, basename.append("COCcontainer").c_str(), py::buffer_protocol());
    }
};
...
struct COC_container_factory<Value_type, false> {
    static auto make(py::module& module, std::string basename) {
        return py::class_<GenericCOC_container<Value_type>>(module, basename.append("COCcontainer").c_str());
    }
};
...
auto expose_coc(py::module& module, std::string basename)
{
    ...
    auto container = COC_container_factory<Value_type, with_buffer_protocol>::make(module, basename);
    container.def("__len__", [](const COC_container &cocc) { return cocc.length(); });

    auto iterator = py::class_<Coc_it>(module, basename.append("COCiterator").c_str());

    auto coc = py::class_<Coc>(module,  basename.append("COC").c_str())
        .def("__iter__", [](Coc& coc) {
        return py::make_iterator(coc.begin(), coc.end());
    },
            py::keep_alive<0, 1>() /* Keep Coc alive while iterator is used */
        )
        .def("__getitem__", [](Coc& coc, int i) -> Coc_cont { return coc[i]; })
