...
auto unpack_well_information(const ComPASS::Well::Well_type well_type)
{
	...
	for(std::size_t wk=0; wk<nb_wells; ++wk) {
		...
		assert(nb_perforations==*(wrapper.well_offset + wk + 1) - *(wrapper.well_offset + wk));
		info.emplace_back( Well_information{
			nb_perforations,
			wrapper.node_vertex + *(wrapper.well_offset + wk),
...
void add_well_wrappers(py::module& module)
{
	...
	py::class_<Perforation_data>(module, "PerforationData")
		...
		.def_readonly("well_index_Fourier", &Perforation_data::well_index_Fourier)
		;

	auto pyWellInfo = py::class_<Well_information>(module, "WellInformation")
		.def_readonly("nb_perforations", &Well_information::nb_perforations)
		.def_property_readonly("vertices", [](const Well_information& self) {
			// Convert to C rank
			const auto n = self.nb_perforations;
			auto result = py::array_t<int, py::array::c_style>{ n };
			const auto p = self.perforations_vertex;
			std::transform(p, p + n, result.mutable_data(0), [](auto i) {return i - 1;});
			return result;
		}, py::return_value_policy::reference_internal)
	;

	auto add_perforation_state_property = [&pyWellInfo](const char * name, std::size_t offset) {
		pyWellInfo.def_property_readonly(name, [offset](py::object& self) {
			...
			auto ptr = reinterpret_cast<const double *>(
				reinterpret_cast<const unsigned char *>(info->perforations_state) + offset					
			);
	...
	add_perforation_state_property("pressure", offsetof(Perforation_state, pressure)) ;
	...
	auto add_perforation_data_property = [&pyWellInfo](const char * name, std::size_t offset) {
		pyWellInfo.def_property_readonly(name,
			[offset](py::object& self) {
				auto info = self.cast<Well_information*>();
				auto ptr = reinterpret_cast<const double *>(
					reinterpret_cast<const unsigned char *>(info->perforations_data) + offset					
				);
				std::vector<std::size_t> shape;
				shape.push_back(info->nb_perforations);
				std::vector<std::size_t> strides;
	...
	add_perforation_data_property("well_index_Darcy", offsetof(Perforation_data, well_index_Darcy)) ;
	add_perforation_data_property("well_index_Fourier", offsetof(Perforation_data, well_index_Fourier)) ;

	auto add_perforation_rank_property = [&pyWellInfo](const char * name, std::size_t offset) {
		pyWellInfo.def_property_readonly(name,
			[offset](py::object& self) {
				auto info = self.cast<Well_information*>();
				...
				for(std::size_t k=0; k+1<nbperfs; ++k, ++p) {
					auto rank = *reinterpret_cast<const int *>(
						reinterpret_cast<const unsigned char *>(ptr) + k * sizeof(Perforation_data)
					);
					assert(rank>0);// Fortran index > 0
					*p = rank - 1;
				}
				assert(*reinterpret_cast<const int *>(
						reinterpret_cast<const unsigned char *>(ptr) + (nbperfs - 1) * sizeof(Perforation_data)
				)==-1); // Well head
				return result;
			});
	};

	add_perforation_rank_property("parent_vertex", offsetof(Perforation_data, parent_vertex_id)) ;
	add_perforation_rank_property("parent_rank", offsetof(Perforation_data, parent_rank)) ;

	py::class_<Well_group_information>(module, "WellGroup")
		.def_property_readonly("nb_wells", [](const Well_group_information& self) {
