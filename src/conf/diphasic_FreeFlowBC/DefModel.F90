...
! Remark: Sg+Sl=1 is not a closure law, as well as Pphase=Pref+Pc(phase) and q^l_atm * S^g = 0, it is forced in the implementation
  ...
  integer, parameter, dimension( NbIncTotalPrimMax, NbContexte) :: &
    psprim = RESHAPE( (/ &
      ...
      T, 3, 8,  & ! DIPHASIC_FF_NO_LIQ_OUTFLOW_CONTEXT=5  P=1, T=2, Cga=3, Cgw=4, Cla=5, Clw=6, Sprincipal=7, freeflow_gas_flowrate=8, freeflow_liquid_flowrate=9
  ...
  integer, parameter, dimension( NbEqFermetureMax, NbContexte) :: &
       ...
       P, 4, 5, 6, 7, & ! DIPHASIC_FF_NO_LIQ_OUTFLOW_CONTEXT=5  P=1, T=2, Cga=3, Cgw=4, Cla=5, Clw=6, Sprincipal=7, freeflow_gas_flowrate=8, freeflow_liquid_flowrate=9
       ! FF dof : LIQUID_OUTFLOW_CONTEXT
