...
void pass_and_dump_dim_array(const double*, ComPASS_Fortran_size_type,
                             const ComPASS_Fortran_size_type*);
void increment_first_column(double*, ComPASS_Fortran_size_type,
                            const ComPASS_Fortran_size_type*);
}
...
void add_wrappers(py::module& module) {
   /**
    CHECKME: we could pass py::array_t::shape() which is a pointer
             to the underlying shape yet we would be wrapping through an array
    of C unsigned longs which have no equivalent in Fortran and we should have a
    compilation time check that sizeof(Py_ssize_t) == sizeof(size_t)
   ...
   */
   ...
   module.def("increment_first_column_in_fortran",
              [fortran_shape](py::array_t<double, py::array::c_style>& a) {
                 auto fshape = fortran_shape(a);
                 increment_first_column(a.mutable_data(), a.ndim(),
                                        fshape.data());
              });
}
