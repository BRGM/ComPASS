...
template <std::size_t ... ks> struct Is_a_range;
...
template <typename ... Ts>
struct Enumerate {
    typedef typename index<Ts...>::template 
        with< range<sizeof...(Ts)> >::type type;
};
...
template <typename Tuple> struct tuple_disjunction;
template <typename ... Ts>
...
template <typename Tuple> struct Is_homogeneous;
template <>
