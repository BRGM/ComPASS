    ...
    def __init__(self, filename):
        ...
        with open(filename) as f:
            ...
            self.vertices = np.array(
                [[float(s) for s in f.readline().split()] for _ in range(nbnodes)],
                dtype=np.double,
            )
            _, n = tagged_int()
            assert n == nbcells
            ...
            assert n == nbfaces
            self.face_cells = [
                [int(s) for s in f.readline().strip().split()] for _ in range(nbfaces)
            ]
            _, n = tagged_int()
            assert n == nbedges
            self.edge_nodes = np.array(
                [[int(s) for s in f.readline().strip().split()] for _ in range(nbedges)]
            )
