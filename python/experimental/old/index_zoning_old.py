...
# TODO: py2to3. replace iteritems() by items() and similar
...
class Zone(object):
    ...
    A zone object is always constructed through its manager, ie. a
    ZoneManager instance. Directly instanciate the Zone class will
    raise an error.

    Zone objects are immutables and hashables, making them valid keys
    for dictionnary.
    ...
    def __array__(self, dtype=None):
        # TODO: gérer dtype en accord avec np.ndarray.__array__()
        assert dtype is None, "Not yet implemented"
        return self.content()
    ...
    def __eq__(self, other):
        ...
        return self is other  # manager is here to ensure this is valid
...
class Zict(dict):
    ...
    A zict behaves exactly as a built-in dict except that its keys
    ...
    # manager defined only by initial setitem
    # @manager.setter  # NOQA
    # def manager(self, value):
    #     if self._manager is not None:
    #         raise Exception("'manager' attribute already set, run clear()"
    #                         "to be able to perform the operation.")
    #     self._manager = value

    @property
    def key_support(self):
        "The union zone of all keys. Or None if empty."
        return Zone.union(*self.keys()) if self else None
    ...
    def _setitem(self, key, value):
        ...
        for z_old in [z for z in self if not z.isdisjoint(key)]:
            z_out = z_old - key
            # self[z_out] = dict.pop(self, z_old)  # before _setitem()
            self._setitem(z_out, dict.pop(self, z_old))  # after _setitem()
        # set item
